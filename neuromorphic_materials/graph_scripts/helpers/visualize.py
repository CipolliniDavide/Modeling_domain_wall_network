...
from . import utils
from .visual import create_colorbar, create_discrete_colorbar
...
def plot_network(
    ...
    if save_path:
        ...
        print('Graph saved to {:s}'.format(save_path))
    if show:
        plt.show()
    else:
        plt.close()
...
def draw_degree_colormap(G, save_name=None, save_fold="./", title=""):
    ...
    _, _, cbar, cmap = create_discrete_colorbar(
        fig=fig,
        ax=ax,
        array_of_values=colors,
        fontdict_cbar_label={"label": "Node degree"},
    )
    # plt.show()
    a = 0
...
def degree_analysis(G, save_name=None, save_fold="./", title=""):
    ...
    try:
        ...
        edge_weight = np.ones(G.number_of_edges())
    try:
        coordinates = [(node, (feat["coord"])) for node, feat in G.nodes(data=True)]
    except:
    ...
    if save_name:
        plt.savefig(join(save_fold, f"{save_name}_DegreeAnalysis.png"))
        plt.close()
    else:
        plt.show()
...
def adjaciency_matx(G, save_name=None, save_fold="./", title="", cbar_label=""):
    ...
    plt.tight_layout()
    if save_name:
        plt.savefig(join(save_fold, f"{save_name}_Adjacency_Matrix.png"))
        plt.close()
    else:
        plt.show()
...
def edge_weight_distribution(
    G, save_name=None, save_fold="./", title="", cbar_label=""
):
    import matplotlib.pyplot as plt
    import networkx as nx
    import numpy as np
...
def histogram(
    bins, values, save_name=None, save_fold="./", title="", ylabel="", xlabel=""
):
    ...
    plt.tight_layout()
    if save_name:
        plt.savefig(join(save_fold, f"{save_name}.png"))
        plt.close()
    else:
        plt.show()
...
def t_SNE_on_embeddings(
    node_embeddings, save_path="/home/hp/Scrivania/", save_name="", random_state=0
):
...
def plot_superpix_image_point_color(
    image,
    segments,
    G,
    attr_color,
    ...
    fig = plt.figure(f"Superpixels -- {num_segments} segments", figsize=(15, 12))
    ax = fig.add_subplot(1, 1, 1)
    font_size = 35
    ax.imshow(mark_boundaries(image, segments, color=(1, 1, 1)), origin="lower")
    # Scatter
    ...
    if cbar_type == "discrete":
        ...
        cbar_ax = make_axes_locatable(ax).append_axes(
            position="right", size="3%", pad=0.1
        )
        # Create colorbar
        print(np.unique(color))
        cbar = fig.colorbar(
            mappable=sc, cax=cbar_ax, ticks=np.arange(np.min(color), np.max(color) + 1)
        )
        ...
        _, _, cbar = create_colorbar(
            fig=fig,
            ax=ax,
            mapp=sc,
            array_of_values=color,
    ...
    if show:
        ...
        plt.savefig(
            join(
                save_fold,
                f"{save_name}-Superpixels-Scatter-{attr_color}-{num_segments}-segments",
            )
        )
        plt.close()
...
def plot_superpix_image_kmeans_point_color(
    embedding, image, segments, G, k=3, save_fold="./", save_name=""
):
    # model.eval()
    # z = model(torch.arange(data.num_nodes, device=device))
    # from sklearn.manifold import TSNE
    ...
    numSegments = len(np.unique(segments))
    fig = plt.figure(
        "Superpixels -Kmeans%d - %d segments" % (k, numSegments), figsize=(18, 12)
    )
    ax = fig.add_subplot(1, 1, 1)
    font_size = 40
    ...
    plt.savefig(
        join(
            save_fold,
            "{}-Superpixels-Scatter-KMeans{:02d}-{:04d}segments.png".format(
                save_name, k, numSegments
            ),
        )
    )
    plt.close()
...
def show_rag(img, segments, g, save_fold="./", save_name="", attr_color=None):
    ...
    # RAG
    lc = graph.show_rag(
        segments, g, img, border_color="yellow", ax=ax, edge_cmap="viridis"
    )
    # Colorbar
    ...
    # Create axis for colorbar
    cbar_ax_sc = make_axes_locatable(ax).append_axes(
        position="bottom", size="3%", pad=0.1
    )
    # Create colorbar
    ...
    plt.tight_layout()
    plt.savefig(join(save_fold, f"{save_name}-RAG"))
    plt.close()


def plot_scatter(
    x_values, y_values, x_label="", y_label="", save_fold="./", save_name=""
):
    ...
    plt.tight_layout()
    plt.savefig(join(save_fold, save_name))
    plt.close()
...
def plot_graph_over_image(G, segments, image, save_fold="./", lw=2.5):
    ...
    fig, ax, cbar = create_colorbar(
        ...
        ax=ax,
        mapp=lc,
        array_of_values=w_list,
        fontdict_cbar_label={"label": "Weights"},
    )
    plt.tight_layout()
    plt.savefig(join(save_fold, "graph_detect.png"))
    plt.close(fig)

    # plt.hist(w_list);
    # plt.savefig(join(save_fold, 'weighs.png'));
    # plt.close()
