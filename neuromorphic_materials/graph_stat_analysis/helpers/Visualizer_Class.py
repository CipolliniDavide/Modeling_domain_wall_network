...
import helpers.angle_extraction
...
class Visualizer:
    ...
    def heatMap_withScale(
        self, save_name=None, save_fold="./", title="", scale_range=None
    ):
        ...
        if x_len is 0:
            ...
            len_str = "%.1f" % x_len
        ax.fill_between(
            x=[x_scale_start, x_scale_start + x_len],
            y1=[self.data.scan_size_micron / 20, self.data.scan_size_micron / 20],
            y2=[
                self.data.scan_size_micron / 20 + self.data.scan_size_micron / 100,
                self.data.scan_size_micron / 20 + self.data.scan_size_micron / 100,
            ],
            color="white",
        )
        ax.text(
            x=x_scale_start + x_len / 3,
            y=self.data.scan_size_micron / 20 + self.data.scan_size_micron / 50,
            s=r"%s $\mathregular{\mu}m$" % len_str,
            va="bottom",
            ha="center",
            color="white",
            size=20,
    ...
    def polar_plot(
        self, bins_number=100, mov_average=1, save_name=None, save_fold="./", title=""
    ):
        """
        source:
        ...
        vmax = self.data.values.max()
        img = ax1.imshow(
            np.flipud(np.rot90(self.data.values, k=1, axes=(0, 1))),
            origin="lower",
            cmap="YlGnBu_r",
            extent=(0, self.data.scan_size_micron, 0, self.data.scan_size_micron),
            vmin=vmin,
        ...
        data = (
            helpers.angle_extraction.extract_pixel_angles_sobel(self.data.values) + 180
        )
        data = np.convolve(
            sorted(list(data)), np.ones(mov_average) / mov_average, mode="valid"
        )
        ...
        sp.plot(angle[:-1], h, linewidth=6, alpha=0.6)
