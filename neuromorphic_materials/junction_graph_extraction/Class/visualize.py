...
class visualize:
    ...
    def adjaciency_matx(G, save_name=None, save_fold="./", title="", cbar_label=""):
        ...
        ax.set_xticklabels(ticks, fontsize=14), ax.set_yticklabels(
            ticks[1:], fontsize=14
        )
    ...
    def plot_k_means_tsn_points(
        data, k=3, save_fold="/home/hp/Scrivania/", save_name="", random_state=0
    ):
        # model.eval()
        ...
        ax.scatter(z[:, 0], z[:, 1], c=node_colors, cmap="jet", alpha=alpha, s=30)
        plt.axis("off")
    ...
    def plot_superpix_image_point_color(
        image, segments, G, attr_color, save_fold="./", save_name="", cbar_type=None
    ):
        import numpy as np
        from skimage.segmentation import mark_boundaries
        ...
        if cbar_type == "discrete":
            ...
            cbar = fig.colorbar(
                mappable=sc,
                cax=cbar_ax,
                ticks=np.arange(np.min(color), np.max(color) + 1),
            )
        ...
        plt.savefig(
            save_fold
            + save_name
            + "-Superpixels-Scatter-%s-%d segments" % (attr_color, numSegments)
        )
        plt.close()
    ...
    def plot_superpix_image_kmeans_point_color(
        ...
        plt.savefig(
            save_fold
            + save_name
            + "-Superpixels-Scatter-KMeans{:02d}-{:04d}segments".format(k, numSegments)
            + ".png"
        )
        plt.close()
    ...
    def plot_graph_over_image(G, segments, image, save_fold="./", lw=2.5):
        ...
        x_coord_edge = np.asarray(
            [[n1[0], n2[0]] for n1, n2, attr in G.edges(data=True)]
        )
        y_coord_edge = np.asarray(
            [[n1[1], n2[1]] for n1, n2, attr in G.edges(data=True)]
        )
