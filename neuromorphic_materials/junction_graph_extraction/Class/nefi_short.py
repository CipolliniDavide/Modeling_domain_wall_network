...
def thinning(args):
    ...
    skeleton = skeletonize(args[0])
    # skeleton = cv2.ximgproc.thinning(cv2.cvtColor(args[0], cv2.COLOR_RGB2GRAY))
...
def breadth_first_edge_detection(skel, segmented, graph):
        ...
        for label, (px, py), nbs in queues:
            for ix, iy in nbs:
                ...
                if value == 0:
                    ...
                    label_length[label] += (
                        1.414214 if abs(ix - px) == 1 and abs(iy - py) == 1 else 1
                    )
    ...
    for l1, l2 in edges:
        ...
        graph.add_edge(
            u,
            v,
            pixels=label_histogram[l1] + label_histogram[l2],
