...
def average_shortest_path_length_for_all(G):
    ...
    if nx.is_connected(G):
        ...
        average = math.fsum(
            len(sg) * nx.average_shortest_path_length(sg) for sg in subgraphs
        ) / sum(len(sg) for sg in subgraphs)
...
def hist_shortest_path(G, save_path):
    ...
    l_list = [
        lengths[node][target] for node in G.nodes() for target in lengths[node].keys()
    ]
    print(len(l_list))
    h, b = plt.hist(l_list, bins=20)
    plt.title(
        "Distribution of  Path Lenght across all Node Pairs| %s"
        % (save_path.split("/")[1])
    )
    ...
    # plt.ylabel('#')
    if save_path:
        plt.savefig(save_path + "shortest_path.png")
    plt.close()
