...
def plot_axes(ax, fig=None, geometry=(1, 1, 1)):
    if fig is None:
        fig = plt.figure()
    if ax.get_subplotspec().get_geometry()[:3] != geometry:
        ax.get_subplotspec().set_geometry(*geometry)
...
def create_discrete_colorbar(
    fig, ax, array_of_values, fontdict_cbar_label=None, fontdict_cbar_ticks=None
):
    fontdict_cbar_label_standard = {"label": None, "fontsize": 35, "fontweight": "bold"}
    if fontdict_cbar_label is not None:
        fontdict_cbar_label_standard.update(fontdict_cbar_label)
    fontdict_cbar_ticks_standard = {"axis": "y", "labelsize": 25}
    if fontdict_cbar_ticks is not None:
        fontdict_cbar_ticks_standard.update(fontdict_cbar_ticks)
    ...
    bounds = np.linspace(
        np.min(array_of_values),
        np.max(array_of_values),
    ...
    cbar_edg = colorbar.ColorbarBase(
        ax2,
        cmap=cmap,
        norm=norm,
        spacing="proportional",
        ticks=bounds + np.ones_like(bounds) * 0.5,
        boundaries=bounds,
        format="%1i",
    )
    ...
    ax2.set_ylabel(
        fontdict_cbar_label_standard["label"], fontdict=fontdict_cbar_label_standard
    )
...
def create_colorbar(
    ...
    if fontdict_cbar_tickslabel is not None:
        fontdict_cbar_tickslabel_standard.update(fontdict_cbar_tickslabel)
    ...
    cbar_edg.ax.set_ylabel(
        ylabel=fontdict_cbar_label_standard["label"],
        fontdict=fontdict_cbar_label_standard,
    )
    cbar_edg.ax.tick_params(**fontdict_cbar_ticks_standard)
    ...
    cbar_edg_ticks_label = [
        valfmt(i, None) for i in cbar_edg_ticks
    ]  # ['%.3f' % i for i in cbar_edg_ticks]
    cbar_edg.ax.set_yticklabels(
        cbar_edg_ticks_label, **fontdict_cbar_tickslabel_standard
    )
